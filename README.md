...
3. Before implementation, repository selection uses explicit repository name/path first, then explicit cross-repository scope, current directory, active file, and finally the sole eligible child. Current-directory and active-file inference resolve the nearest canonical Git root and use it only when that exact root was discovered, so an undiscovered nested repository never selects its parent. Explicit scope overrides editor context; ambiguous requests at a multi-repository container root ask instead of guessing. Invalid, stale, linked-worktree, `.claude/worktrees`, nested-undiscovered, outside-workspace, unselected, and unconfirmed-root targets are rejected before lifecycle Git actions. For each selected repository, the skill creates and checks out its own `feature/<feature-id>` branch in that repository's existing primary checkout; it never creates a Git worktree. Immediately before source edits, the delegate must verify its actual canonical working directory, Git root, `.git` directory, recorded branch, and base relationship match that primary checkout. A host-created worktree or other mismatch is refused before edits without automatic relocation or repair; the plugin detects this host runtime mismatch but cannot disable host worktree isolation itself. If that feature branch is already occupied in another existing worktree, it blocks instead. If the target already exists and the tree is dirty, it blocks before checkout; destructive reset requires separate confirmation. Backend and frontend engineers receive one workspace-relative repository assignment at a time and cannot infer or touch siblings.
