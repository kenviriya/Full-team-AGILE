...
7. Each ticket state records its artifact path, immutable ID and slug, repository path/key, builder role, `dependsOn`, referenced PRD criteria, status, dependency depth, dependency-PR guidance, branch, base commit and source, runtime, worktree ownership, changed files, checks, commit SHA, push remote/ref/status, blockers, temporary artifacts, QA/review outcomes, and cleanup outcome. Ticket prose stays in its Obsidian artifact; State.md keeps compact execution data.
...
A ticket with no dependencies branches from the repository's recorded feature base commit. A dependent ticket branches from the exact recorded committed tip of its prerequisite ticket branch after that prerequisite has passed its required implementation gate. Verify that the tip differs from or validly descends from the prerequisite base and record the full SHA. Never transfer uncommitted changes, merge, or rebase, and never substitute the primary checkout HEAD or another ticket's tip.
...
Immediately before every source edit, Git mutation, QA run, review, or cleanup, validate the selected repository, recorded ticket runtime, canonical Git root, exact branch, base ancestry, worktree registration when applicable, and operation boundary. File tools use absolute paths beneath the runtime; Git uses `git -C <recorded-runtime>`; non-Git commands may use only command-local `cd -- <recorded-runtime> && ...`. Reject `..`, symlink, canonical, or command escapes. Expected uncommitted ticket changes are allowed before commit and for QA/review but not branch creation, switching, push, or optional deletion.

After a builder pass completes its required checks, commit and push from the recorded ticket runtime. Fail closed unless the current branch exactly equals the recorded `feature/<ticket-id>-<slug>` branch and is not `main`, `develop`, the repository return branch, or any configured protected branch. Reject detached HEAD and every sibling, parent, target, or otherwise different branch. Refuse unrelated changes outside ticket ownership; remove registered temporary artifacts, stage only the ticket's reported owned paths with explicit pathspecs, inspect the staged diff, and create a normal non-amended commit. Record the full commit SHA before pushing.

Push only to `origin` after verifying that remote exists, using `git -C <recorded-runtime> push -u origin HEAD:refs/heads/<recorded-ticket-branch>`. Revalidate the exact runtime and branch immediately before push, require a clean worktree whose HEAD is the recorded commit, and never push another local or remote ref. Record the remote, exact ref, and result. A missing remote, authentication failure, rejection, or divergence blocks delivery; never force-push, retry with a broader refspec, or redirect to another branch.
...
Never create or inspect pull requests, poll CI, merge, rebase, tag, release, delete a branch, or commit/push anywhere except the validated recorded ticket runtime and exact ticket branch above.
...
Every builder, QA, and review delegation receives exactly one ticket: State.md reference, ticket artifact, approved PRD, optional UI spec, ticket ID, referenced criteria, dependency/base data, one selected workspace-relative repository, canonical primary root, recorded runtime, exact branch/base commit, allowed ownership, ticket Git-delivery rules, and ticket-specific evidence. Native Agent cwd is untrusted. The delegate verifies all recorded boundaries and never infers or touches siblings, other tickets, or another runtime.

Builders implement only their assigned ticket. After successful checks they commit and push only through the ticket Git-delivery contract, then report ticket ID, changed files, checks, commit SHA, push result, blockers, temporary-artifact requests, and evidence for each referenced PRD criterion. A failed ticket returns to the same builder ticket; the PM does not create a fix ticket.
...
- **ready-for-integration:** enter only after every ticket is committed, pushed, independently passes QA and review, and completes required cleanup. Do not call the feature integrated or done.

## Ready for integration

List every approved ticket ID, artifact, repository path, branch, base commit/source, commit SHA, push remote/ref/result, QA/review evidence, and cleanup outcome in deterministic order. State explicitly that these independently approved ticket branches are **not an integrated feature tree** and that integrated validation remains pending external user-managed PRs, CI, dependency-order merges, and final integration testing. The feature directory is the audit trail. Do not direct the release workflow to release this feature until external integration is complete.
