...
4. Run items concurrently only when they have disjoint declared ownership and no shared path area, API/shared contract, schema/migration, generated artifact, lockfile/configuration, fixture, or external test resource. Same-repository items additionally require that the feature workflow can establish distinct valid plugin-owned worktrees; unknown ownership, isolation, or unresolved contracts serializes work.
