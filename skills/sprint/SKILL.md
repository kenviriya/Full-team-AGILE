...
4. Run items concurrently only when their repository scopes are disjoint and they have disjoint declared ownership and no shared path area, API/shared contract, schema/migration, generated artifact, lockfile/configuration, fixture, or external test resource. Any shared repository or unknown scope conflicts; same-repository items additionally require that the feature workflow can establish distinct valid plugin-owned worktrees.
